...
#   Author: bustami2020
#   Blog: https://www.kangarif.net/
#   Github - v2-ui: https://github.com/bustami2020/v2-ui
#======================================================
...
install() {
    bash <(curl -Ls https://github.com/bustami2020/v2-ui/raw/master/v2-ui.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/bustami2020/v2-ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
